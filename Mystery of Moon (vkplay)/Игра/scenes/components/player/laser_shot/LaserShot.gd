...
onready var _dotted_line_node := $DottedLine as Node2D
onready var _dotted_line_pulse_node := $DottedLine/Pulse as AnimationPlayer
...
func _on_Charge_frame_changed() -> void:
    if _weapon_charge_anim_node.animation == 'heating':
        if _weapon_charge_anim_node.frame == _CHARGE_ANIMATION_COUNT: # Стадия выстрела.
            is_shooting = true
            is_fuel_consumer = false
            _dotted_line_pulse_node.play('shot')
        ...
        is_shooting = false
        is_fuel_consumer = false
        _dotted_line_pulse_node.play('pulse')
...
func destruction() -> void:
    _is_charge_resetting = false
    _weapon_charge_anim_node.stop()
    set_visibility_dotted_line(false)


func set_visibility_dotted_line(is_visible: bool) -> void:
    if is_visible:
        _dotted_line_node.show()
    else:
        _dotted_line_node.hide()
