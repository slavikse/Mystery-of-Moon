...
var _menu_audio_playback_position := -1
var _game_audio_playback_position := -1

var _is_menu_showed := false
...
onready var _ray_sphere_node := $OptionsContainer/Background3/PlayerContainer/RaySphere as AnimatedSprite
...
func _process(_delta: float) -> void:
    ...
    if _player_node.ext_is_airdrop and Input.is_action_just_pressed('ui_menu'):
        pause_switcher()
...
func pause_switcher(puzzle_number := -1) -> void:
    _is_menu_showed = not _is_menu_showed
...
func _on_Toggler_animation_finished(_anim_name: String) -> void:
    if not _is_menu_showed:
        layer = -1


func show_menu() -> void:
    _is_menu_showed = true
