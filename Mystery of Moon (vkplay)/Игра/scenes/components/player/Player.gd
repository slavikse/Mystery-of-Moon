...
const LASER_FUEL_CONSUMPTION := SHIELD_FUEL_CONSUMPTION * 4.0
...
const CAMERA_ZOOM_CUSTOM := Vector2(0.6, 0.6)
...
onready var body_destroy_node := $Body/Destroy as Particles2D
...
onready var fuel_refill_front_node := $Fuel/RefillFront as Particles2D
onready var fuel_refill_back_left_node := $Fuel/RefillBackLeft as Particles2D
onready var fuel_refill_back_right_node := $Fuel/RefillBackRight as Particles2D

onready var recovery_recover_front_node := $Recovery/RecoverFront as Particles2D
onready var recovery_recover_back_node := $Recovery/RecoverBack as Particles2D
onready var recovery_refill_front_node := $Recovery/RefillFront as Particles2D
onready var recovery_refill_back_left_node := $Recovery/RefillBackLeft as Particles2D
onready var recovery_refill_back_right_node := $Recovery/RefillBackRight as Particles2D

onready var energy_shield_node := $EnergyShield as EnergyShield
onready var laser_shot_node := $LaserShot as Node2D # LaserShot
onready var _added_ability_front_node := $AddedAbility/AddedAbilityFront as Particles2D
onready var _added_ability_back_node := $AddedAbility/AddedAbilityBack as Particles2D
onready var _added_ability_node := $AddedAbility/AddedAbility as Particles2D
...
func ext_player_restore() -> void:
    ...
    laser_shot_node.is_enabled = bool(GC.ext_player_state.is_laser_shot_enabled)
    #warning-ignore:UNSAFE_METHOD_ACCESS
    laser_shot_node.set_visibility_dotted_line(true)

    #warning-ignore:UNSAFE_PROPERTY_ACCESS
    if laser_shot_node.is_enabled:
        #warning-ignore:UNSAFE_METHOD_ACCESS
        laser_shot_node.toggle_show(true)
        menu_node.show_sphere('ray')

    recovery_recover_front_node.emitting = true
    recovery_recover_back_node.emitting = true
    recovery_refill_front_node.emitting = true
    recovery_refill_back_left_node.emitting = true
    recovery_refill_back_right_node.emitting = true
...
func _calc_bad_ending() -> void:
    #warning-ignore:UNSAFE_PROPERTY_ACCESS
    GC.is_bad_ending = len(GC.puzzles_state) >= _PUZZLE_NUMBERS_FOR_BAD_ENDING and (control_manager_node.ext_energy_shield_enabled or laser_shot_node.is_enabled)
...
func save_game() -> void:
    GC.ext_save_game({
        ...
        #warning-ignore:UNSAFE_PROPERTY_ACCESS
        "is_laser_shot_enabled": laser_shot_node.is_enabled,
...
func laser_shot(delta: float) -> void:
    #warning-ignore:UNSAFE_PROPERTY_ACCESS
    #warning-ignore:UNSAFE_PROPERTY_ACCESS
    if laser_shot_node.is_enabled and laser_shot_node.is_fuel_consumer:
        fuel -= LASER_FUEL_CONSUMPTION * delta

        if fuel <= 0.0:
            #warning-ignore:UNSAFE_METHOD_ACCESS
            laser_shot_node.stop_weapon_charge()
...
func fuel_refill() -> void:
    ...
    fuel_refill_back_left_node.emitting = true
    fuel_refill_back_right_node.emitting = true
...
func game_over(type: int) -> void:
    ...
    #warning-ignore:UNSAFE_METHOD_ACCESS
    laser_shot_node.destruction()
    #warning-ignore:UNSAFE_METHOD_ACCESS
    laser_shot_node.toggle_show(false)
...
func switch_camera_zoom() -> void:
    if is_jumping_gameplay:
        camera_node.zoom = camera_node.zoom.linear_interpolate(CAMERA_ZOOM_CUSTOM, JUMPING_GAMEPLAY_SWITCH_TIME)
        camera_node.offset = camera_node.offset.linear_interpolate(CAMERA_OFFSET_CUSTOM, JUMPING_GAMEPLAY_SWITCH_TIME)
    else:
        camera_node.zoom = camera_node.zoom.linear_interpolate(Vector2.ONE, JUMPING_GAMEPLAY_SWITCH_TIME)
...
func _on_Player_area_entered(area: Area2D) -> void:
    ...
    if area is AltarSphere:
        ...
        elif int(altar_container_node.AltarType.LASER_SHOT) == altar_container_node.altar_type:
            #warning-ignore:UNSAFE_PROPERTY_ACCESS
            laser_shot_node.is_enabled = true
            #warning-ignore:UNSAFE_METHOD_ACCESS
            laser_shot_node.toggle_show(true)
            _configure_added_ability('ray', altar_container_node)
            _calc_bad_ending()
...
func _configure_added_ability(sphere: String, altar_container_node: AltarContainer) -> void:
    ...
    if sphere == 'shield':
        ...
        _added_ability_back_node.modulate = _ADDED_ABILITY_SHIELD_COLOR

    elif sphere == 'ray':
        _added_ability_front_node.modulate = _ADDED_ABILITY_RAY_COLOR
        _added_ability_back_node.modulate = _ADDED_ABILITY_RAY_COLOR

    _added_ability_front_node.emitting = true
    _added_ability_back_node.emitting = true
