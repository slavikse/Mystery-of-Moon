...
func _ready() -> void:
    ...
    if window:
        window.VKWebAppViewHide = JavaScript.create_callback(self, "VKWebAppViewHide")
        window.VKWebAppViewRestore = JavaScript.create_callback(self, "VKWebAppViewRestore")


func VKWebAppViewHide():
    _menu_node.for_vkgames_audio_playing(false)


func VKWebAppViewRestore():
    _menu_node.for_vkgames_audio_playing(true)
...
func _on_ShareWithFriends_pressed() -> void:
    var message := "'Я играю в #Mystery_of_Moon! от разработчика #PixSynt!'"
